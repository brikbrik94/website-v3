...
- **Never fix/change `oe5ith-ci` or `geodata-plugin-standard` from this repo.** Both submodules are maintained externally with their own review process. If a bug, schema gap, or desired extension is found while working here, **file it as a GitHub Issue directly on the target repo** (`brikbrik94/oe5ith-ci` or `brikbrik94/geodata-plugin-standard`) with the full write-up (context, root cause/motivation, real repro/example from this repo, concrete proposal) — solvable independent of which machine you're on next, unlike a local-only file. Then add a **short tracking entry** here (issue link + a compact checklist of the requested fields/behavior, not a duplicate of the issue body) to `docs/ci/open-items.md` / `docs/ci/bug-reports.md` or `docs/geodata/open-items.md` / `docs/geodata/bug-reports.md` respectively — re-check the checklist against the submodule's new state after every `git submodule update`. Reference from `TODO.md`/`ROADMAP.md` with a short pointer, not a full description. (Convention changed 2026-08-12 — older `docs/ci/*-request.md`/`docs/ci/handoff-*.md` files predate this and were filed as local-only text, not GitHub Issues.)
