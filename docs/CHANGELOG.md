...
### Hinzugefügt
- **Valhalla als zweiter Routing-Provider neben ORS** — Provider-Umschalter in der
  Routing-Sidebar (A→B-Modus inkl. deutscher Turn-by-Turn-Wegbeschreibung mit Abbiege-Symbolen,
  sowie SEW-/NEF-Matrixsuche für die nächstgelegenen Stützpunkte), live-Statusanzeige des
  gewählten Providers. Ursprünglich als reine Testumgebung begonnen, dann zur vollen
  Provider-Parität mit ORS ausgebaut.
- **Routing-Endpoints konsolidiert:** ein einziger, provider-parametrisierter Proxy
  (`routing-proxy.php`) ersetzt die bisher getrennten Endpoints. ORS/Nominatim laufen jetzt über
  interne Serveradressen statt öffentlicher Subdomains — ein separater API-Key ist dafür nicht
  mehr nötig.
- **Kartenlegende auf neues Schema (v3.0) umgestellt:** geteilte Farbskalen (z.B. volle
  Pisten-Schwierigkeitsgrad-Skala), generischer Symbol-Fallback für Icon-Gruppen,
  Linienbreite/Strichmuster/Umrandung (Casing) in den Legenden-Symbolen, Chip-Streifen für
  Zeilen mit mehreren Farbvarianten in einer Kategorie (z.B. Skipisten), echte Transparenz statt
  immer volldeckender Flächen-Symbole, sowie Zusammenfassung mehrfach vorkommender einfarbiger
  Overlay-Zeilen (z.B. Autobahnen) zu einer Zeile.

### Geändert
- **`oe5ith-ci`-Designsystem auf v2.0.0 aktualisiert** — Turn-by-Turn-Wegbeschreibungs-Symbole
  sind jetzt hochformatig (16×24 statt 16×16), rein optische Anpassung.
- **Legenden-Panel für Chip-Streifen-Zeilen verbreitert**, damit auch Kategorien mit vielen
  Farbabstufungen (z.B. Pistenschwierigkeit) lesbar bleiben.

### Behoben
- **Zwei nach der Routing-Endpoint-Umstellung übersehene Stellen** zeigten fälschlich einen
  Fehler bzw. meldeten den ORS-Dienst dauerhaft als „down" (Graph-Export-Werkzeug,
  Systemstatus-Anzeige im Info-Bereich) — korrigiert.
- **ORS-Anfragen schlugen auf dem internen Server fehl** (fehlender Pfadanteil in der internen
  Serveradresse) — korrigiert.
- **Legenden-Chip-Streifen** brachen bei mehr als 4 Farben unschön um und saßen optisch leicht
  über statt auf gleicher Höhe mit der Beschriftung — beides korrigiert.

### Sicherheit
- **Öffentliche Routing-Endpoints bekommen eine einheitliche Zugriffsbeschränkung** (Rate-Limit
  pro Besucher + Referer-Prüfung) — schließt eine zuvor bestehende Lücke beim
  Valhalla-Matrix-Zugriff, der bisher ganz ohne eigenen Schutz war.
- **Fehlerantworten der Routing-Endpoints geben keine rohen technischen Details mehr an den
  Client weiter.**
