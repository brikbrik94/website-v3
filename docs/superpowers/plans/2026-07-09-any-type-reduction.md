...
- **`.test.ts`-Dateien bleiben unangetastet** (43 der 121 Fundstellen, laut ROADMAP.md bewusst ausgenommen — Mocking-`any` ist dort vertretbar). **Präzisierung (nach Task 2 entdeckt):** Das gilt nur fürs Chasen von `any` *innerhalb* eines Testfiles. Bricht eine Produktions-Typänderung dieser Task die Kompilierung eines Testfiles (weil es z.B. ein absichtlich unvollständiges Literal direkt an eine jetzt strikter typisierte Funktion übergibt), ist das minimale Nachziehen dieser Aufrufstelle (z.B. ein präziser `as SourceSpecification`/`as LayerSpecification`-Cast auf das Literal) Teil der Task, die die Änderung verursacht hat — kein separates any-Hunting, nur so viel wie nötig für grünen `tsc`. Bereits vorhandene `any`-Mocks im selben Testfile bleiben unangetastet.
