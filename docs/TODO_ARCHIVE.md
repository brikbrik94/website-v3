...
## 2026-08-30 — Performance-Baseline-Audit: 2 Punkte archiviert (nächster Durchlauf startet neu, nicht Fortsetzung)

Beim TODO-Review als "später erneut angehen" eingestuft — der nächste Performance-Testdurchlauf
fängt komplett neu an (kein Fortsetzen dieser beiden Einzelpunkte), daher hier archiviert statt
in TODO.md offen gehalten. Bei Bedarf erneut aufmachen, sobald ein neuer Audit-Durchlauf
stattfindet.
...
## 2026-08-30 — Map-Subsystem: Anschlussfeatures (kompletter Abschnitt archiviert, alle Punkte erledigt)

Ursprünglich von ROADMAP.md nach TODO.md verschoben (2026-07-09), jetzt komplett nach hier
verschoben (2026-08-30) — der letzte offene Punkt des Abschnitts (`legendItemsRefCount`-Bug,
siehe eigener Archiv-Eintrag oben) wurde beim TODO-Review als gegenstandslos identifiziert, womit
keine offenen Punkte mehr übrig waren. Ursprünglicher Abschnittstext (unverändert):
...
## 2026-08-30 — legendItemsRefCount-Gruppierungsbug: gegenstandslos durch Schema-v3.0-Migration

- [x] **`MapPage.ts`s `legendItemsRefCount` gruppiert falsch, wenn zwei Gruppen desselben
  Overlays unterschiedliche `legend_items` tragen** (gefunden 2026-08-12, hier archiviert
  2026-08-30 — nicht durch einen Code-Fix, sondern durch externe Schema-Migration
  gegenstandslos geworden). Beim TODO-Review live gegen `https://tiles.oe5ith.at/layers.json`
  verifiziert: die drei ursprünglich als Repro genannten Overlays (`openskimap`, `zonen-nef`,
  `zonen-sew`) sind inzwischen alle auf Schema-Version **3.0** — deren `groups[]` tragen dort
  gar keine `legend_items`/`color`/`type`/`legend_scale_id`-Felder mehr (das ist jetzt über den
  neuen `legend[]`-Top-Level-Block gelöst, `MapPage._syncV3Legend()`). Eine vollständige
  Durchsuchung der aktuellen `layers.json` ergab: **keine einzige Gruppe** in der gesamten Datei
  trägt noch `legend_items`. Die Vorbedingung des Bugs (zwei gleichzeitig aktive Gruppen
  desselben Overlays mit unterschiedlichen `legend_items`-Sätzen) kann mit dem heutigen
  Live-Backend also nicht mehr auftreten. Der betroffene Code-Pfad
  (`legendItemsRefCount`/`event.legendItems` in `src/pages/MapPage.ts`) existiert weiterhin im
  Repo (toter Pfad für den Fall, dass ein Overlay je wieder auf ein Pre-3.0-Schema zurückfiele),
  wurde aber bewusst nicht entfernt — außerhalb des Scopes dieses Reviews. Ursprünglicher
  TODO-Text (unverändert):

  **Vorbestehender Bug** — nicht durch die `render`/`variants`-Runde eingeführt, nur
  generalisiert (Ref-Zählungs-Key war vorher ausschließlich `overlayId`, dann
  `legendGroupKey ?? overlayId`; das Problem bestand für den `overlayId`-Fall unverändert).
  `toggleLayer()` zählte aktive Gruppen pro `groupKey` (`legendItemsRefCount`) und rendert
  `event.legendItems` nur beim Übergang 0→1 aktiver Gruppen — das setzt voraus, dass alle
  Gruppen eines `groupKey` denselben `legend_items`-Satz tragen. Live gegen
  `https://tiles.oe5ith.at/layers.json` verifiziert (2026-08-12), dass das damals bereits nicht
  zutraf: `openskimap` hatte die Gruppen „Ski-Spots" (6 `legend_items`) und „Lifte"
  (7 `legend_items`), beide ohne `legend_scale_id` → beide bekamen `groupKey = "openskimap"`.
  Ebenso `zonen-nef` (6 Gruppen, je 7-15 `legend_items`) und `zonen-sew` (8 Gruppen, je
  23-63 `legend_items`). Werden zwei solche Gruppen eines Overlays gleichzeitig aktiviert: (a)
  nur die Legenden-Zeilen der zuerst aktivierten Gruppe rendern (0→1-Gate lässt die zweite
  Aktivierung nichts mehr anzeigen), und (b) hinterlässt ein Abschalten der zuerst aktivierten
  Gruppe orphaned Legenden-Zeilen, die nie wieder entfernt werden. **Bewusst nicht sofort
  behoben** — ein echter Fix hätte einen Content-Hash- oder anderen kollisionsfreien Key statt
  reinem `groupKey`-Ref-Count gebraucht.

## 2026-08-27 — Valhalla-Connector-Rollout: Historie archiviert (Hauptpunkt war bereits ERLEDIGT)

- [x] **Valhalla-Connector: vor Live-Deploy** (2026-08-23 umgesetzt, 2026-08-27 archiviert). Beim
  TODO-Review gegen den Live-Code verifiziert, dass alle unten offen gebliebenen Unterpunkte
  inzwischen tatsächlich aufgelöst sind: `api/valhalla.php` existiert nicht mehr (nur noch
  `api/valhalla-client.php`, reine `valhalla_call()`-Funktion), `api/routing-proxy.php` ersetzt
  es; `nginx.conf` hat kein `deny all;` mehr, sondern denselben `limit_req`+`valid_referers`-Block
  auf **beiden** `location = /api/routing-proxy.php` UND `location = /api/nearest-stations.php`;
  `VALHALLA_URL` hat bewusst keinen Produktiv-Default in `api/config.php` (muss über
  `config.local.php` gesetzt werden, wie in `config.local.php.example` dokumentiert). Ursprünglicher
  Text (vollständige Historie, unverändert) folgt:
  ...
  **Scope seit 2026-08-22 gewachsen:** `?path=route` liefert inzwischen volle Turn-by-Turn-Daten
  ...
  - [x] Authentifizierung/Rate-Limiting auf `api/valhalla.php` (aktuell: offener Proxy, sobald
        `VALHALLA_URL` gesetzt ist) — moot: `valhalla.php` existiert nicht mehr, `routing-proxy.php`
        hat das nginx-`limit_req` stattdessen.
  - [x] Ob der `deny all;`-Block in `nginx.conf` als alleiniger Schutz reicht oder `valhalla.php`
        zusätzlich aus dem `deploy-website.sh`-rsync ausgeschlossen werden soll — moot: Datei
        gelöscht.
  - [x] Ob `VALHALLA_URL` weiterhin eine private Tailscale-IP bleibt oder ein öffentlich
        erreichbarer Endpoint nötig wird — falls Tailscale: sicherstellen, dass der Produktivserver
        selbst im Tailnet hängt — entschieden: dauerhaft öffentlich nutzbarer Bestandteil der
        Produktivkonfiguration.
  - [x] `nearest-stations.php?provider=valhalla` erreicht Valhalla ohne den nginx-`deny all;` von
        `valhalla.php` — braucht eine eigene Auth-/Rate-Limiting-Entscheidung, nicht nur die für
        `valhalla.php` gedachte — gelöst: derselbe nginx-Block sitzt jetzt auch auf
  ...
  **Update 2026-08-23:** Die SEW/NEF-Matrixsuche für Valhalla ist jetzt umgesetzt — nicht über
  ...
  Verlass gelten. **(Historisch — `valhalla.php` existiert seit dem Routing-Endpoint-Umbau
  2026-08-23 nicht mehr, dieser Absatz beschreibt einen zwischenzeitlichen, inzwischen aufgelösten
  Zustand.)**

## 2026-08-27 — Performance-Baseline: /tracking-TBT-Ausreißer nicht reproduzierbar

- [x] **`/tracking`: auffällig hoher TBT (5.840 ms) gegenüber den übrigen 5 Seiten (1.990–3.320
  ms)** (2026-08-11 gefunden, 2026-08-11 als nicht reproduzierbar eingestuft, 2026-08-27
  archiviert). `mainthread-work-breakdown`-Audit zeigte 9,7 von 12,0 s Mainthread-Arbeit in der
  nicht weiter attribuierten Kategorie „Other". Details: `docs/performance/2026-07-28-baseline-audit.md`,
  Befund 6.
  ...
  **Nicht reproduziert (2026-08-11):** erneuter `npm run perf:audit`-Lauf zeigt `/tracking` bei
  ...
  während des Audits), kein stabiler Befund. Archiviert statt weiter als offener TODO-Punkt
  geführt, da kein zweiter Auffälligkeits-Lauf seit 2026-08-11 aufgetreten ist — bei einem neuen
  Live-Befund erneut aufmachen statt hier weiterzuschreiben.
